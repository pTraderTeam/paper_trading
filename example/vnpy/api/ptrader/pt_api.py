...
class PTraderApi:
    ...
    def get_url(self, method_name: str):
        """生成url"""
        return "/".join([self.home_url, method_name])

    def connect_server(self, host: str, port: int):
        """连接模拟交易程序"""
        self.server_ip = host
        self.server_port = str(port)
        self.home_url = "http://" + ":".join([self.server_ip, self.server_port])
    ...
    def get_pos_record(self, start, end, save_data=False):
        ...
        if status:
            if isinstance(pos_record, list):
                ...
                pos_df = pos_df[
                    ["pt_symbol", "max_vol", "first_buy_date", "last_sell_date", "buy_price_mean", "sell_price_mean", "profit", "is_clear", "account_id"]
                ]
                if save_data:
    ...
    def show_orders_record(self, order_df):
        ...
        order_df = order_df[["order_date", "order_time", "order_type", "order_price", "trade_price", "volume"]]
            ...
            if row["status"] == "全部成交":
                ...
                if row["order_type"] == "buy":
                    ax_canddle.annotate(
                        ...
                        arrowprops=dict(facecolor="r", alpha=0.3, headlength=10, width=10),
                    )
                else:
                    ax_canddle.annotate(
                        "S",
                        ...
                        arrowprops=dict(facecolor="g", alpha=0.3, headlength=10, width=10),
                    )
    ...
    def show_pos(self, pos_list: list):
        ...
        if pos_list:
            ...
            pos_df = pos_df[["pt_symbol", "buy_date", "volume", "available", "buy_price", "now_price", "profit"]]
